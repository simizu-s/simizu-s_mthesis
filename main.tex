...
\textcolor{blue}{コードクローンとは，プログラムテキスト中の一致または類似するコード片である~\cite{Inoue2001}.
...
\textcolor{blue}{現状では，ソースコード中のコードクローン検出は，単一のツールを用いて行われる．
...
\textcolor{blue}{この問題解決のため，本論文では，コードクローン検出後の後処理として統一的なラベリング手法を提案する．
...
\textcolor{blue}{本研究では，評価実験として被験者実験を行った．
被験者実験では，コードクローンの大規模データセットであるBigCloneBench~\cite{BigCloneBench}に含まれるクローンペアを対象とし，
BigCloneBenchによる分類と，提案手法による分類のどちらが人間の感覚に近いかを調査した．
その結果，BigCloneBenchでType3に分類され，提案手法でType4と分類されたクローンペアについては，
BigCloneBenchの分類の方が，人間の感覚に近いことがわかった．
一方，BigCloneBenchでType4に分類され，提案手法でType3と分類されたクローンペアについては，
提案手法による分類の方が，人間の感覚に近いことがわかった．
これらの結果から，分類の定義について修正を行う必要性が示唆された．
定義の修正を行うことで，人間の感覚により近い分類が可能となる．
また，被験者ごとの傾向から，Type3およびType4のコードクローンの分類の定義の曖昧さが示唆され，
本研究の目的である，統一的なラベリング手法の必要性が改めて確認された．
}

\textcolor{blue}{以降，2章では，本研究で用いる技術について述べる．
3章では，本研究の動機について述べる．
4章では，提案手法の詳細について述べる．
...
トークンの類似度計算に基づいたラベリングを行う．

% 2. 構文構造（AST）単位の手法
\textcolor{blue}{2.4節で述べる抽象構文木を利用する手法として，Deckard~\cite{Jiang2007}が挙げられる．
...
この手法は構文的な根拠に基づいた比較を行うため，分類に応じたラベリングが可能である．}
...
\textcolor{blue}{既存のコードクローンデータセットを表~\ref{tab:codeclone_dataset}に示す．
BigCloneBench~\cite{BigCloneBench}は，Javaプロジェクトを対象とした大規模なデータセットである．
詳細については，2.3.1項で述べる．}

% % 2．意味的クローンおよび多言語への対応
\textcolor{blue}{近年では，同一の処理を行うものの，構文が大きく異なるType4に特化したデータセットや，
多言語を対象としたデータセットが作成されている．
SemanticCloneBench~\cite{SemanticCloneBench}は，Java, Python, C, C\#の4言語にまたがる4,000件のペアを提供する．
また，GPTCloneBench~\cite{GPTCloneBench}は，近年の大規模言語モデル（LLM）の進展を背景に構築された新しいデータセットであり，
Java, Python, C\#における3万件以上のペアが含まれる．}

% % 3．機能等価性と検証用オラクル
\textcolor{blue}{特定の言語において高い精度で検証を行うためのデータセットも存在する．
FEMPD~\cite{FEMPDataset,HigoMSR2022}はJavaの機能等価なメソッドペアに焦点を当てており，高精度な解析が求められる研究で利用されている~\cite{InoueSERA2024,SomeyaICSME2025}．
...
FEMPD~\cite{FEMPDataset,HigoMSR2022} & Java & 1,342 \\ \hline
\end{tabular}
\end{table}
...
\textcolor{blue}{BigCloneBenchに含まれるクローンペアは，いずれもメソッド単位のコード片から構成される．}

BigCloneBenchでは，Type1およびType2のコードクローンは，ソースコードの正規化後テキストの一致を確認することで判定している．
一方，Type3およびType4のコードクローンは，行単位での構文的類似度に基づいて0以上1未満の範囲で，次のように分類される．
...
BigCloneBenchをType-4クローンの検出性能評価に用いる場合，誤った分類の影響で結果の妥当性が脅されるため，
Type4クローンの検出を目標とする，機械学習を用いたコードクローン検出の評価結果に対する信頼性が低下する~\cite{KrinkeIWSC2022,Krinke2025}．
...
\textcolor{blue}{BigCloneBenchに含まれるそれぞれのTypeに分類されたクローンペアの数は表~\ref{tab:BCB_numbers}に示す通りである．}
...
\textcolor{blue}{本章では，既存のコードクローン検出手法の問題点，
...
例えば，CCFinder~\cite{KamiyaTSE2002}，NIL~\cite{NIL}は，\textcolor{blue}{ソースコードを
トークン列に変換して比較する，字句単位の解析を行う．
Nicad~\cite{Nicad}は行単位で解析を行う．これは，識別子の正規化やコードの整形といった前処理を適用した後の各行を比較することでコードクローンを検出する．}
...
\textcolor{blue}{既存のコードクローンのデータセットとして最大のものはBigCloneBench~\cite{BigCloneBench}である．
BigCloneBenchでは，Type1およびType2のコードクローンは，ソースコードの正規化後テキストの一致を確認することで判定している．
...
\textcolor{blue}{本研究では，BigCloneBenchの各ペアに対し，行の一致率と字句単位の類似度の相関を調査した．
...
\textcolor{blue}{算出結果を図~\ref{fig:line_to_lcs}に示す．
...
% これは，特定の処理が少数の長い行に記述されているか，あるいは多数の短い行に分散しているかといった，表面的な記述形式の違いによって分類が左右されていることを示している．
}

\textcolor{blue}{以上の知見は，
行単位や字句単位の一致率に基づく既存の分類基準が，コードクローンの分類指標として不十分であることを示唆しており，
既存の分類基準に代わる新たな分類基準が必要であることを示す．
したがって，提案手法では，ASTに基づいた新たなType分類の定義を用いる．
ASTに基づいた分類基準を設けることで，ソースコードの表面的な記述形式に左右されず，プログラムの論理的な構造を反映した一貫性のある分類が可能となる．
}
...
\textcolor{blue}{提案手法の分類定義におけるif文，while文等の制御構造を持つソースコードの扱いについて述べる．
図~\ref{fig:AST_if}にif文のJavaプログラムと対応するASTを示す．
簡単のため，一部省略している．
この二つのコードでは，赤く示したif文の条件式の比較演算子が異なる．
ASTで，葉ノードおよび単文以下のノードとして除かれるノードを灰色で示す．
if文の条件式部分は単文として除かれないため，この例はType4と分類される．
}


\begin{figure}[!t]
  \centering
  \includegraphics[width=0.8\textwidth]{images/ast_if.pdf}
  \caption{if文の例}
  \label{fig:AST_if}
\end{figure}
...
\textcolor{blue}{分類には，AppleM3チップ(8コアCPU)，メモリ24GBのMacBookAirを使用した．}
Javaのバージョンは25.0.1を利用した．
提案手法を用いてBigCloneBench内のクローンペアの分類を行った結果を表~\ref{tab:BCB_type}に示す．
\textcolor{blue}{分類}に要した総時間は35時間49分であった．
...
\begin{itemize}
    \item \textcolor{blue}{BigCloneBenchで}Strongly-Type3に分類されるクローンペアのうち提案手法でType4と分類されたクローンペア
    \item \textcolor{blue}{BigCloneBenchで}Weakly-Type3に分類されるクローンペアのうち提案手法でType3と分類されたクローンペア
\end{itemize}

被験者には，事前に，2.1節に記述したコードクローン分類の定義を提示し，100個のクローンペアがどちらに分類されるべきかを判断してもらった．
また，\textcolor{blue}{判断に迷った場合にその理由と最終的な判断の理由についても回答してもらった．}
...
28.3\%がType4と判断され，\textcolor{blue}{提案手法による分類}が人間の感覚に反するという結果になった．
一方，Weakly-Type3に分類されるクローンペアのうち提案手法でType3と分類されたクローンペアは，
71.0\%がType3と判断され，\textcolor{blue}{提案手法による分類}が人間の感覚に近いという結果になった．
また，全体的にType3と判断されたクローンペアの数が多かった．

\textcolor{blue}{提案手法と同じ判断をした被験者の人数ごとクローンペアの分布を図~\ref{fig:result_experimet_violin}に示す．
また，提案手法と同じ判断をした被験者の人数ごとのクローンペアの数を表~\ref{tab:result_plot}に示す．
Strongly-Type3に分類されるクローンペアのうち提案手法でType4と分類されたクローンペアは，
分布の偏りから，判断に迷いが少ないことがわかる．
一方，Weakly-Type3に分類されるクローンペアのうち提案手法でType3と分類されたクローンペアは，
BigCloneBenchの分類を選んだのが3人，提案手法の分類を選んだの3人である場合，つまり意見が割れたクローンペアが
多く存在することから，判断の迷いがあったと考える．
}

それぞれについて，被験者による判断と\textcolor{blue}{提案手法による分類}が一致した場合，異なった場合のコードを確認し，分析を行った．
以降に詳細を述べる．
...
Strongly & 0 & 0 & 2 & 9 & 14 & 22 & 3 \\ 
Weakly & 9 & 16 & 8 & 15 & 0 & 2 & 0 \\ \hline
% 時間　&  & 9:55 min &  40.8 s & 4:34 min & 10:29 min & 1 d 11:23 h \\ \hline
\end{tabular}
\end{table}

\subsubsection{Strongly-Type3に分類されるクローンペアのうち提案手法でType4と分類されたクローンペア}
\textcolor{blue}{本項では，BigCloneBenchによってStrongly-Type3と分類されたものの，提案手法ではType4と判定されたクローンペアについて詳細を述べる．}
...
次に，被験者全員が提案手法と異なるType3と判断したクローンペアの例を示す．
ソースコード~\ref{code:experimet_pair34}は，強調表示されたwhile文の条件式においてASTに差異が生じたため，提案手法ではType4と判断された．
しかし，被験者の判断では，条件式のみの変更は，文の変更に相当し，Type3の定義の範疇に収まるとしてType3と判断していた．
現在の実装では，while文，if文，for文等の制御構造において，内包される条件式のAST構造が異なる場合は一律にType4と判定される．
...
\begin{minipage}{\textwidth}
    ...
    \def\@captype{lstlisting}
    \makeatother
    \caption{被験者全員が提案手法と異なるType3と判断したクローンペアの例(引数の違い)}
...
ソースコード~\ref{code:experimet_pair57}は，メソッドの\textcolor{blue}{仮引数}の数が異なっていたため，
ASTに差異が生じ，提案手法ではType4と判断された．
しかし，被験者は，引数の増減もType3における文の変更の一部であると判断した．
引数の数や順序の変更についても，条件式と同様に，特定のノード間での差異を許容するアルゴリズムを導入することで，
Type3として分類が可能である．
...
ソースコード~\ref{code:experimet_pair11}は，被験者全員が提案手法と同じType3と判断したクローンペアの例である．
この例では，互いに共通する文を多く含んでおり，被験者はその内容の類似性からType3と判断したと考えられる．
...
一方，ソースコード~\ref{code:experimet_pair55}は，被験者6人のうち5人が提案手法と異なるType4と判断した例である．
この例では，機能は同一であるが，構文的に異なるとして被験者はType4と判断していた．
...
\textcolor{blue}{被験者ごとのType3と判断したペア数を表~\ref{tab:user_type3_numbers}に，
被験者間の回答の一致率を表~\ref{tab:user_agreement}に示す．
}
被験者6人の回答を分析した結果，その判断基準は大きく2つの傾向に類別できる．
具体的には，評価の際に構文構造の差異を重視する立場と，処理機能の同一性を重視する立場の違いである．
前者のグループは，2.1節のType3の定義に基づき，クローンペア間で異なる部分が，文の挿入，削除，変更に留まる場合にはType3と判断し，
それ以上の構文の変更が見られる場合にType4と判断していた．
一方，後者のグループは，2.1節のType4の定義に基づき，同一の処理を行う場合にType4，行わない場合にType3と判断していた．
...
\label{tab:user_agreement}
% \small % 表全体の文字サイズを少し小さく調整
\begin{tabular}{lcccccc}\hline
 & P1 & P2 & P3 & P4 & P5 & P6 \\ \hline
P1 & 100\% & 57\% & 64\% & 75\% & 55\% & 63\% \\
P2 & 57\% & 100\% & 51\% & 54\% & 76\% & 74\% \\
P3 & 64\% & 51\% & 100\% & 63\% & 39\% & 45\% \\
P4 & 75\% & 54\% & 63\% & 100\% & 46\% & 52\% \\
P5 & 55\% & 76\% & 39\% & 46\% & 100\% & 78\% \\
P6 & 63\% & 74\% & 45\% & 52\% & 78\% & 100\% \\ \hline
% \addlinespace[0.5em]
% \multicolumn{7}{l}{\footnotesize ※ P1--P6は各被験者の実験回答データを示す。} \\
\end{tabular}
\end{table}

% P1: ta1kmr，: ry-inoue: i-kondou: g-kimura: c-yosida: tk-kawam

\subsection{考察}

6.1.1項の結果から，提案手法の厳密な構造比較はType4の特定に寄与する一方で，
人間が文のバリエーションとして許容する範囲をType4と過剰に判定する傾向があることが示唆された．
...
実験の結果，このような規模の著しい乖離がある場合には，Type3と分類することが人間の直感から離れる原因になると示唆された．
したがって，単文以下のノードを除く際，何らかの制約を設ける必要があると考えられる．
具体的な改善策として，以下の2点が考えられる．
\begin{itemize}
