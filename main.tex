...
Type4クローンの検出を目標とする，機械学習を用いたコードクローン検出の評価結果に対する信頼性が低下する．
...
BigCloneBenchでは，Type1およびType2のコードクローンは，ソースコードの正規化後テキストの一致を確認することで判定している．
一方，Type3およびType4のコードクローンは，行単位での構文的類似度に基づいて0以上1未満の範囲で，
Strongly-Type3，Moderately-Type3，Weakly-Tyoe3の3種類に分類される．
この構文的類似度は，行の一致する割合で求められる．
この分類について，
}

\textcolor{blue}{BigCloneBenchに含まれるStrongly-Type3，Moderately-Type3，Weakly-Type3のコードクローンについて，
トークンごとに比較を行い，一致率を計算した．
トークン単位の類似度算出にあたっては，二つのトークン列間における最長共通部分系列(LCS: Longest Common Subsequence)~\cite{Hunt1977,Wagner1974}を用いた．
...
それぞれについて，被験者による判断と\textcolor{blue}{提案手法による分類}が一致した場合，異なった場合，被験者の中で判断が分かれた場合についてコードを確認し，分析を行った．
以降に詳細を述べる．
...
%     \caption{被験者六人中四人がType4と判断したクローンペアの例}
%     \label{code:experimet_pair56}
% \end{figure}
...
これらの例に対し，提案手法がType3と判定した要因は，単文以下のノードを除いた後のASTの形状が同一であったためである．
