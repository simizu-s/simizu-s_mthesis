...
既存のJava言語を対象として研究では，AST生成にEclipse JDT\footnote{https://github.com/eclipse-jdt/eclipse.jdt.core}
...
例えば，CCFinder~\cite{KamiyaTSE2002}，NIL~\cite{NIL}は，\textcolor{blue}{字句単位の比較を行うことで検出を行う．}
Nicad~\cite{Nicad}は\textcolor{blue}{行単位の比較を行うことで検出を行う．}これは，識別子の正規化やコードの整形といった前処理を適用した後の各行を比較することでコードクローンを検出する．
...
\textcolor{blue}{しかし，類似度という数値指標に基づく分類は，解析の単位粒度に結果が大きく左右される．
...
以上の知見は，
\textcolor{blue}{行単位や字句単位の一致率の類似度に基づく既存の分類基準が不安定であることを示唆しており，
...
本手法において，演算子を葉ノードの抽象化対象から除外した理由は，\textcolor{blue}{計算処理の種類を}厳密に区別するためである．
Type2のコードクローンは識別子やリテラルの置換を許容するが，演算子の変更は\textcolor{blue}{計算手順そのものの変容を意味する．}
...
\textcolor{blue}{提案手法の分類定義におけるif文，while文，for文等の制御構造を持つソースコードの扱いについて述べる．
...
提案手法と同じ判断をした被験者の人数ごとクローンペアの分布を図~\ref{fig:result_experimet_violin}に示す．
...
\textcolor{blue}{Strongly-Type3に分類されるクローンペアのうち提案手法でType4と分類されたクローンペア，
...
\textcolor{blue}{表~\ref{tab:result_plot}に示されているように，}
...
現在の定義では，while文，if文，for文等の制御構造において，内包される条件式のAST構造が異なる場合は一律にType4と判定される．
...
\textcolor{blue}{しかし，BigCloneBenchではWeakly-Type3に分類され，Type4として扱われ，人間の感覚と反する．}
...
\textcolor{blue}{人間が2.1節に示したType3の定義として許容する範囲のコードクローンを
Type4と判定する場合があることが示唆された．
}
...
\begin{itemize} 
    \item 単文の数による量的制約：
    \textcolor{blue}{追加，削除または変更された単文の数に閾値を設け，コードの分量が著しく異なる場合には，Type3判定しないように制限する方法である．
    例えば，挿入された文の数が元の構造に対して一定の割合を超える場合，それをType3ではなくType4と判定する．
    } 
    \item 文の種類や文脈に応じたフィルタリング：
    \textcolor{blue}{ソースコード中の全ての単文を一律に取り除くのではなく，文の内部構造(例：メソッド呼び出しの有無)に応じて，
    その単文を除くか，保持するかを判定する方法である． 
    変更を許容する要素を文脈に応じて動的に選択することで，単なる変数への代入といった軽微な実装の差異は除き，
    外部ライブラリの呼び出しを伴う重要なロジックの変更を保持することが可能となる．
    }
\end{itemize}
...
\textcolor{blue}{表~\ref{tab:user_agreement_heatmap}の被験者間の一致率に基づき，
被験者を二つのグループに分類した．
...
これら二つのグループについて判断理由の回答を分析した結果，両者の間でクローン判定の基準が明確に異なることが判明した．
}
具体的には，評価の際に構文構造の差異を重視する立場と，処理機能の同一性を重視する立場の違いである．
グループAは，2.1節のType4の定義に基づき，同一の処理を行う場合にType4，行わない場合にType3と判断していた．
...
考察で述べた通り，被験者の判断基準には「構文重視」と「機能重視」の二つの傾向が確認された．
実験前の教示においてクローンの定義を説明しているが，被験者個人の経験や直感に起因する判断のブレを完全に排除することは困難である．
...
特定のドメインや特定の開発手法（テストコード等）におけるクローン特性が，本手法の分類精度に影響を与える可能性については今後の検討課題である．
...
\acknowledgement{
    ...
    I would like to express my deepest gratitude to Professor Raula from the Department
    ...
    大阪大学大学院情報科学研究科コンピュータサイエンス専攻肥後研究室のメンバーには，
    被験者実験の参加や普段の日常生活において，様々な時間を共にしました．心より深く感謝申し上げます．
    最後に，研究活動に関わらず，多くのサポートをしていただきました事務職員 軽部 瑞穂
    氏に心より深く感謝申し上げます．
}
