...
BigCloneBench~\cite{BigCloneBench}は，Java プロジェクトを対象とした世界最大規模のデータセットである．
800万件を超える膨大なクローンペアを収録しており，Type1 から Type4 までを網羅していることから，
多くのクローン検出ツールの評価においてデファクトスタンダードとして利用されている．
本研究においても，手法の有効性を検証するための主要なデータソースとして活用する．

% % 2．意味的クローンおよび多言語への対応
近年では，字面や構文が大きく異なるType4に特化したデータセットや，
多言語を対象としたものが登場している．SemanticCloneBench~\cite{SemanticCloneBench}は 
Java, Python, C, C\# の4言語にまたがる 4,000 件のペアを提供し，意味的等価性の評価に主眼を置いている．
また，GPTCloneBench~\cite{GPTCloneBench}は，近年の大規模言語モデル（LLM）の進展を背景に構築された新しいデータセットであり，
Java, Python, C\# における 3万件以上のペアを収録している．

% % 3．機能等価性と検証用オラクル
特定の言語において高い精度で検証を行うためのデータセットも存在する．
FEMPD~\cite{FEMPDataset,HigoMSR2022}は Java の機能等価なメソッドペア（Functional Equivalent Method Pairs）
に焦点を当てており，高精度な解析が求められる研究で重用されている．
一方で，Clone oracle~\cite{CloneOracle}は 66 件と小規模ではあるが，C/C++ における厳密な正解（オラクル）を提供しており，
手法の基本的な正当性を検証する際の重要なリファレンスとなっている．


\begin{table}[!b]
\centering
\caption{コードクローンのデータセット}
...
分類にはMacBookAirを使用し，AppleM2チップ(10コアCPU)，メモリに24GBを搭載している．
Javaののバージョンは25.0.1を利用した．
提案手法を用いてBigCloneBench内のクローンペアの分類を行った結果を表~\ref{tab:BCB_type}に示す．
データセットの構築に要した総時間は35時間49分であった．
...
28.3\%がType4と判断され，提案手法が人間の感覚に反するという結果になった．
一方，Weakly-Type3に分類されるクローンペアのうち提案手法でType3と分類されたクローンペアは，
71.0\%がType3と判断され，提案手法が人間の感覚に近いという結果になった．
また，全体的にType3と判断されたクローンペアの数が多かった．
それぞれについて，被験者による判断と提案手法が一致した場合，異なった場合，被験者の中で判断が分かれた場合についてコードを確認し，分析を行った．
以降に詳細を述べる．

\subsubsection{Strongly-Type3に分類されるクローンペアのうち提案手法でType4と分類されたクローンペア}
本項では，提案手法によってType4と分類されたものの，被験者による評価ではType3と判定されたクローンペアについて詳細を述べる．
...
ソースコード~\ref{code:experimet_pair56}は被験者6人中4人が提案手法と同じType4と判断したクローンペアの例である．
この例では，一方がwhile文，他方がfor文という異なる制御構造を含んでいるため，
...
しかし，被験者の判断では，条件式のみの変更はの変更は文の変更に相当し，Type3の定義の範疇に収まるとしてType3と判断していた．
現在の実装では，while文，if文，for文等の制御構造において，内包される条件式のAST構造が異なる場合は一律にType4と判定される．
今後は，制御構文内の条件式における軽微な変更を許容するよう判定ロジックを修正することで，より人間の直感に近い分類が可能になると考えられる．
...
ソースコード~\ref{code:experimet_pair57}は，メソッド呼び出しにおける引数の数が異なっていたため，
...
本項では，Weakly-Type3に分類されるペアのうち，提案手法によってType3と判定されたケースについて述べる．
