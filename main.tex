...
ソースコードの規模が大きくなると，ソースコード中のコードクローンの量も増加するため，手動でコードクローンを管理することは困難となる．そのため，ソースコード中から自動的にコードクローンを検出するツールが研究されている~\cite{Jiang2007,KamiyaTSE2002,Nicad}．
...
\textcolor{blue}{同じソースコードからコードクローンを検出する場合でも検出されるコードクローンが異なったり，同じ箇所がコードクローンとして検出された場合でもその分類が異なったりする．
その結果，コードクローン検出ツールの性能比較を目的としたツール間の比較や，機械学習モデルの学習に必要な高精度なデータの提供が困難となる．}
また，コードクローン検出ツールには，検出のみを行い，分類を行わないツールも存在する．それらのツールについても，Typeごとの比較を行うことはできない．
本研究では，基準を明確にした統一的なラベリングを行うことで，ツール間での比較を可能にし，機械学習の学習データとして提供するの質の高いデータの作成を目的としている．

\subsection{BigCloneBench}
% 既存の大規模なベンチマークとその問題点
BigCloneBenchは，コードクローン検出器の性能評価で用いられるJavaの大規模なベンチマークである~\cite{BigCloneBench}．
\textcolor{blue}{複数のソフトウェアプロジェクトから収集された大規模リポジトリ群であるIJDataset2.0から，特定の機能を実装する可能性のあるソースコードを自動で特定し，
対象の機能を正しく実装しているか否かのタグ付が手動で行われている．}

BigCloneBenchでは，Type3及びType4のコードクローンを，行単位での構文的類似度に基づいて0以上1未満の範囲で，以下のように分類している．
...
既存のコードクローン検出ツールや機械学習を用いたコードクローン検出の研究において，Weakly-Type3クローンはType4クローンとして用いられている~\cite{AlamICSME2025,PinkuICSME2024,WangICSE2023,YuICSE2025,ZhangICCQ2024}．
...
\begin{description}
    ...
    \item[根ノード] 親を持たないノードを指す~\cite{Fan2021ICSE}．図~\ref{fig:AST_discription}では青く示されたprogramノードである．
    \item[葉ノード] 子を持たないノードを指す~\cite{Fan2021ICSE}．図~\ref{fig:AST_discription}では緑で示された二つのノードである．斜体で示されているのがそのノードの持つ値である．
\end{description}
...
定義は以下の通りである．
前提としてすでにコードクローンとして検出されたコードが対象で，
...
それぞれのノードの名前と，斜体で示された値が完全に同一である場合，Type1と判断される．
...
黒く示されたノードの名前が同一であるため，Type2と判断される．
...
それ以下のノードが除かれる．黒く示されたノードの名前が同一であるため，Type3と判断される．
...
代入文 & x = a + b; \\
メソッド呼び出し文 & System.out.println(s);\\
ローカル変数宣言文 & int count = 0; \\
